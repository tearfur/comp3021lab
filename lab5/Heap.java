...
public class Heap<T extends Comparable<T>> {
    ...
    public T poll() {
        ...
        T ret = container.remove(container.size() - 1);
        heapifyDown();
        return ret;
    }
    ...
    private void heapifyUp() {
        for (int i = container.size() - 1; container.get(i).compareTo(container.get(getParentIndex(i))) < 0; i = getParentIndex(i))
    ...
    private void emptyEx() {
        if (container.isEmpty())
            throw new IllegalStateException(this.getClass().getName() + ": This container is empty");
    }
}
